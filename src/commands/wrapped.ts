...
function printWrapped(data: WrappedData): void {
  ...
  console.log(colors.bold(`║   ${colors.bgMagenta(' myvcs wrapped ')} - ${monthName} ${data.period.year}                         ║`));
  console.log(colors.bold('╚═══════════════════════════════════════════════════════════════╝'));
  console.log();
  ...
  console.log(colors.dim(`  Generated by myvcs wrapped | ${new Date().toLocaleDateString()}`));
  console.log();
}
...
function printAvailablePeriods(periods: { year: number; month: number }[]): void {
  ...
  if (periods.length === 0) {
    ...
    console.log(colors.dim('  Start using myvcs to build your wrapped!'));
  } else {
    // Group by year
    ...
    for (const [year, months] of byYear) {
      ...
      for (const month of months.sort((a, b) => b - a)) {
        console.log(`    ${MONTH_NAMES[month - 1]} ${colors.dim(`(myvcs wrapped ${year} ${month})`)}`);
      }
    }
  }
...
export async function handleWrapped(args: string[]): Promise<void> {
  ...
  if (!userId) {
    throw new TsgitError(
      'Not authenticated. Run `myvcs up` to start the platform and log in.',
      ErrorCode.OPERATION_FAILED,
      ['Run `myvcs up` to start the platform', 'Log in with `myvcs token create`']
    );
  }
  ...
  try {
    ...
    if (!data) {
      ...
      console.log(colors.dim('    - Use myvcs to track your coding activity'));
      console.log(colors.dim('    - Try `myvcs wrapped --list` to see available periods'));
      console.log(colors.dim('    - Try `myvcs wrapped --previous` for last month'));
      console.log();
      return;
    }
...
function printHelp(): void {
  console.log(`
${colors.bold('myvcs wrapped')} - Your monthly coding activity summary

${colors.cyan('USAGE')}
  myvcs wrapped [options] [year] [month]
...
${colors.cyan('EXAMPLES')}
  myvcs wrapped                 Show current month's wrapped
  myvcs wrapped --previous      Show last month's wrapped  
  myvcs wrapped 2024 11         Show November 2024 wrapped
  myvcs wrapped --list          List all available periods
