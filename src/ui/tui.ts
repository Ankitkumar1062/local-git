...
async function importOpenTUI() {
  try {
    // We use new Function to bypass tsc's transpilation to require()
    // By keeping it as a native dynamic import(), Bun can natively load the ESM module!
    const core = await new Function("return import('@opentui/core')")();
    return core;
  } catch (error) {
...
export class TsgitTUI {
  ...
  // UI State
  private currentView: 'main' | 'log' | 'diff' | 'branches' | 'stacks' | 'input' | 'help' = 'main';
  ...
  private setupKeyBindings(): void {
    ...
    keyHandler.on('keypress', (key: KeyEvent) => {
      ...
      if (this.currentView === 'help') {
        ...
        // Restore diff view content
        if (this.selectedFile) {
          this.showFileDiff(this.selectedFile);
        } else {
          this.diffText.content = 'Select a file to view diff';
...
Press any key to close`;
  ...
  private quit(): void {
    ...
    // Explicitly emit terminal reset sequence to prevent garbage characters in bash (disables Alternate Screen & Mouse Tracking)
    process.stdout.write('\x1b[?1049l\x1b[?25h\x1b[?1000l\x1b[?1002l\x1b[?1003l\x1b[?1015l\x1b[?1006l\x1b[0m\r\n');
...
export async function launchTUI(): Promise<void> {
  // If not running in Bun, we must fork to Bun because @opentui/core requires bun:ffi
