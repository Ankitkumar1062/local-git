...
export function createOrgRoutes(): Hono {
  ...
  app.get('/:org', async (c) => {
    ...
    if (!orgName) {
      return c.json({ message: 'Organization name is required' }, 400 as 200 | 201 | 400 | 401 | 403 | 404 | 405 | 422 | 500);
    }
  ...
  app.put('/:org/memberships/:username', requireAuth, requireScopes('org:write'), async (c) => {
    ...
    if (!orgName || !username) {
      return c.json({ message: 'Organization and username are required' }, 400 as 200 | 201 | 400 | 401 | 403 | 404 | 405 | 422 | 500);
    }
