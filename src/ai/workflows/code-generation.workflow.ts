...
const analyzeTaskStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    // Detect project type using myvcs's file reading
    ...
    // Find relevant patterns in the codebase using myvcs APIs
    const relevantPatterns: string[] = [];
    try {
      // Look for common patterns using myvcs's file listing
...
const searchConventionsStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      // Analyze file naming conventions using myvcs APIs
      const allFiles = await findFilesInRepo(inputData.repoPath, /\.(ts|tsx)$/);
      ...
      // Check for test patterns using myvcs file listing
      ...
      // Find example files for the task using myvcs file listing
 ...
 * 
 * Generated by myvcs AI Code Generation Workflow
 * Task: ${inputData.task}
 */
...
const applyAndValidateStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    // Apply files using myvcs APIs (unless dry run)
    if (!inputData.dryRun) {
    ...
    try {
      ...
      if (inputData.createPR && commitSha) {
        try {
          ...
          // Get the default branch using myvcs API
      ...
      console.error('[Code Generation] myvcs operations failed:', error);
      return {
        success: false,
        generatedFiles: inputData.appliedFiles,
        validation: inputData.validation,
