...
const parseDiffStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      // Get the diff using myvcs's native APIs
...
const createReviewStep = createStep({
  ...
  execute: async ({ inputData }) => {
    ...
    try {
      ...
      const botUser = await userModel.findByUsername('myvcs-bot');
...
function formatReviewAsMarkdown(result: {
  ...
  lines.push('---');
  lines.push('*This review was generated by myvcs AI PR Review Workflow.*');
